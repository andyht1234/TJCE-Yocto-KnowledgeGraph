...
url = 'https://lore.kernel.org/yocto-docs/'
url_content = ''
previous_title = ''
num = 0
folder_path = ''
...
while url:
    ...
    if pre_tags:
        for link in pre_tags[0].find_all('a'):
            href = link.get('href')
            if '#nav' not in href:
                url_content = 'https://lore.kernel.org/yocto-docs/' + href
        ...
        if len(pre_tags) > 1:
            ...
            if next_link:
                url = "https://lore.kernel.org/yocto-docs/" + next_link.get('href')
            else:
                url = None
        else:
            url = None
