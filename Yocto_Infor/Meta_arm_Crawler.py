...
url = 'https://lore.kernel.org/yocto-meta-arm/'
url_content = ''
previous_title = ''
num = 0
folder_path = ''
...
if(num1 == '0'):
    folder_path = 'meta_arm_TXT'
else:
    folder_path = 'meta_arm_JSON'
...
while url:
    ...
    if pre_tags:
        for link in pre_tags[0].find_all('a'):
            href = link.get('href')
            if '#nav' not in href:
                url_content = 'https://lore.kernel.org/yocto-meta-arm/' + href
        ...
        if len(pre_tags) > 1:
            ...
            if next_link:
                url = "https://lore.kernel.org/yocto-meta-arm/" + next_link.get('href')
            else:
                url = None
        else:
            url = None
