...
url = 'https://lore.kernel.org/yocto-meta-virtualization/'
url_content = ''
previous_title = ''
num = 0
folder_path = ''
...
if(num1 == '0'):
    folder_path = 'meta_virtualization_TXT'
else:
    folder_path = 'meta_virtualization_JSON'
...
while url:
    ...
    if pre_tags:
        for link in pre_tags[0].find_all('a'):
            href = link.get('href')
            if '#nav' not in href:
                url_content = 'https://lore.kernel.org/yocto-meta-virtualization/' + href
        ...
        if len(pre_tags) > 1:
            ...
            if next_link:
                url = "https://lore.kernel.org/yocto-meta-virtualization/" + next_link.get('href')
            else:
                url = None
        else:
            url = None
